...
def do_canny(frame): #灰度-高斯-canny边缘
    # 边缘检测
...
def do_segment(frame):
    # 区域遮罩
    polygons = np.array([  #左上 右上 右下 左下
        [(50, 300), (540, 300), (540, 360), (50, 360)]
    ])  # 创建数组构建多边形选定感兴趣区
...
def do_color(img):  # 对车道线进行轮廓识别
    ...
    img = img.copy()  # 复制图像
    cv.drawContours(img, conts, -1, (0, 255, 0),
                    1)  # 轮廓绘制，第一个参数是指明在哪幅图像上绘制轮廓，第二个参数是轮廓本身，第三个参数指定绘制轮廓list中的哪条轮廓，如果是-1，则绘制其中的所有轮廓
    return img
...
# cap = cv.VideoCapture("1.mp4")  # 把素材视频放到对应地址
...
#         cv.putText(frame, "FPS {0}".format(float('%.1f' % (counter / (time.time() - start_time)))), (50, 50),
#                    cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255),
#                    3)  # 文字绘制
#         cv.imshow("initial", image_resize(frame))  # 展示初始图像
...
def process_image(image_path):
    ...
    center = image_resize(center)  # 调整中心点检测图像的大小

    # 显示和保存结果
    plt.figure(figsize=(10, 10))
    plt.subplot(231), plt.imshow(cv.cvtColor(img, cv.COLOR_BGR2RGB)), plt.title('Original')
    plt.subplot(232), plt.imshow(canny, cmap='gray'), plt.title('Canny Edges')
    plt.subplot(233), plt.imshow(segment, cv.COLOR_BGR2RGB), plt.title('Segmented')
    plt.subplot(234), plt.imshow(color, cv.COLOR_BGR2RGB), plt.title('Color Detection')
    plt.subplot(235), plt.imshow(center, cv.COLOR_BGR2RGB), plt.title('Center')
    plt.show()
