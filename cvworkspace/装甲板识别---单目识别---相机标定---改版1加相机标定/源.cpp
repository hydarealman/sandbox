...
int image_detect(Mat hero_image, Mat real_cameraMatrix, Mat real_distCoeffs) {
    return 0;
}

double calculate_php(Mat cameraMatrix,Mat distCoeffs,float meanLen, float dis,Point center) {
    //获取装甲板的四个点的像素坐标
...
//绘制代码
int draw_picture(Point center,Mat frame,double depth) {
    //绘制代码
    std::ostringstream depthStr;
    depthStr << std::fixed << std::setprecision(2) << "depth:" << depth << "m";//将深度值转换为字符串
    cv::Point textPos(center.x - 40, center.y - 20);//确定文字显示位置-30和-10是经验偏移值,目的是让文字显示在装甲板中心的上方偏左的位置
...
//装甲板识别
int armoured_plate_recognize(Mat frame) {
    //变量集中定义
    ...
    for (size_t i = 0; i < lightInfos.size(); i++) {
        ...
        draw_picture(center,frame,depth);
...
int main() {
    ...
    //相机标定
    camera_calibration(cameraMatrix, distCoeffs);
    for (;;) {
        video.read(frame);  //读取每帧
        if (frame.empty()) {
            break;
        }
        armoured_plate_recognize(frame);
    }
    video.release();
    //writer.release();
